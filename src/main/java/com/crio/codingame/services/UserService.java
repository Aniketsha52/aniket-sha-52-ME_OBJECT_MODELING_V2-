...
public class UserService implements IUserService {
    ...
    public List<User> getAllUserScoreOrderWise(ScoreOrder scoreOrder){
      ...
      List<User> userOrder = userRepository.findAll();
      
      if(ScoreOrder.ASC == scoreOrder){   
        userOrder.sort(Comparator.comparing(User::getScore));
        
      }

      if(ScoreOrder.DESC == scoreOrder){
        userOrder.sort(Comparator.comparing(User::getScore).reversed());
      }
      return userOrder;
    }
